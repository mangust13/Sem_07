        ...
        public IActionResult Process(EncryptViewModel model)
        {
            ...
            try
            {
                ...
                if (model.Mode == "encrypt")
                {
                    ...
                    // === RSA ===
                    var rsa = new RSAEncryptor(); // створює нову пару ключів
                    var rsaTimer = Stopwatch.StartNew();
                    byte[] sample = System.IO.File.ReadAllBytes(inputPath).Take(200).ToArray(); // невеликий блок
                    ...
                    // зберігаємо ключі для подальшого дешифрування
